...
- `mem import --from-md <path>` — advisory-only import: parses a markdown file (CLAUDE.md-style) for `-`/`*` bullet-list lines that look like preference/decision-shaped statements and stores each as a **`pending`, `source_type: "derived"`** fact through the exact same `captureSuggested` trust path as any other suggested candidate — there is no code path that writes `active`, and no bulk-promote shortcut; every imported fact still requires an explicit `mem review --promote <id>`. Bullets nested under an obviously structural heading (e.g. `Architecture`, `File Structure`) are skipped; everything else errs toward importing, since `pending` status is the real safety gate, not pre-filtering. Provenance (`<absolute file path>:<line>`) is recorded in the fact's existing `source_ref` field. `--dry-run` reports the candidates that would be imported (count + preview) without writing anything. Re-importing the same file skips bullets already imported at the same `source_ref` + text (no duplicate facts).
